...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: `RESOURCE CREATED: Id= ${resourceId}, Name= ${resourceName}, Price= ${resourcePrice}, Unit= ${resourcePriceUnit} `,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `RESOURCE CREATE FAILED: Name= ${resourceName} is already used`,
        entityType: "resource",
        entityId: null,
      });
      ...
      return res.status(409).json({
        ...
        details: "A resource with this name already exists. Please choose another name.",
      });
    ...
    await logEvent({
      actorUserId,
      message: `RESOURCE CREATE FAILED VALIDATION: missing or invalid fields for Name= ${resourceName}`,
      entityType: "resource",
      entityId: null,
    });
    return res.status(400).json({
      ...
      details: "Please fill in all required fields correctly.",
    });
  }
